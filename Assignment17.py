...
bt = Button(window,text="EXIT",width=35,command=sys.exit)
bt.pack()
window.mainloop()
...
from tkinter import *
def show():
    print("my name is keshav rana!")
root = Tk()
root.title("My Text")
bt = Button(root,text="click here!",width=30,height=10,command=show)
bt.place(x=1,y=1)
bt.pack()
root.mainloop()
...
def show():
    print("keshav rana")
    label.config(text = "i am back")
create = Tk()
label = Label(create,text="My Project",width=35,font='20',bg="purple",fg="white")
label.pack()
button1 = Button(create,text="Label changed",width=20,height=5,bg="red",command = show)
button1.pack()
button2 = Button(create,text="Exit",width=10,height=2,command=sys.exit)
button2.pack()
create.mainloop()
...
label1.grid(row=1)
bt = Button(root,text="PRINT",width=15,command=show)
